...
onStatus _merchant booking (Booking dOrder) = do
  ...
  whenJust booking.partnerOrgId $ \_ -> fork "updating status of tickets in google wallet for bookingId" $ traverse_ updateStatesForGoogleWallet googleWalletStates
  ...
  where
    ...
    updateStatesForGoogleWallet (ticketNumber, state') = do
      ...
      resp <- try @_ @SomeException $ void $ GWSA.getObjectGoogleWallet serviceAccount resourceId
      case resp of
        Left _ -> return ()
        Right _ -> do

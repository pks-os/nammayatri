...
getObjectGoogleWallet :: (Metrics.CoreMetrics m, MonadFlow m, HasFlowEnv m r '["googleSAPrivateKey" ::: String]) => TC.ServiceAccount -> Text -> m ()
getObjectGoogleWallet sa resourceId = do
  ...
  url <- parseBaseUrl "https://walletobjects.googleapis.com"
  void $ callAPI url eulerClient "Calling Google Wallet API" (Proxy @GetWalletAPI) >>= fromEitherM (FailedToCallWalletAPI . show)

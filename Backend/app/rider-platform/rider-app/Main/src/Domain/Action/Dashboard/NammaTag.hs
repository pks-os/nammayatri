...
postNammaTagUpdateCustomerTag merchantShortId opCity userId req = do
  ...
  when (req.isAddingTag && maybe False (Yudhishthira.elemTagNameValue req.tag) person.customerNammaTags) $ throwError (InvalidRequest "Tag already exists")
  ...
  let tag =
        if req.isAddingTag
          then do
            ...
            addCustomerTag person.customerNammaTags reqCustomerTagWithExpiry
          else removeCustomerTag person.customerNammaTags req.tag
  QPerson.updateCustomerTags (Just tag) personId
  pure Success

-- TODO move to lib
addCustomerTag :: Maybe [LYT.TagNameValueExpiry] -> LYT.TagNameValueExpiry -> [LYT.TagNameValueExpiry]
addCustomerTag Nothing tag = [tag]
addCustomerTag (Just tags) tag = tags ++ [tag] -- FIXME what if tag duplicated?

removeCustomerTag :: Maybe [LYT.TagNameValueExpiry] -> LYT.TagNameValue -> [LYT.TagNameValueExpiry]
removeCustomerTag Nothing _ = []
removeCustomerTag (Just tags) tag = filter ((/= tag) . Yudhishthira.removeTagExpiry) tags

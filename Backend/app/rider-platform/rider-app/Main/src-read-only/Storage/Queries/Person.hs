...
updateCustomerTags customerNammaTags id = do
  _now <- getCurrentTime
  updateWithKV [Se.Set Beam.customerNammaTags ((fmap (.getTagNameValueExpiry) <$>) customerNammaTags), Se.Set Beam.updatedAt _now] [Se.Is Beam.id $ Se.Eq (Kernel.Types.Id.getId id)]
...
updateByPrimaryKey (Domain.Types.Person.Person {..}) = do
  ...
  updateWithKV
    [ Se.Set Beam.aadhaarVerified aadhaarVerified,
      ...
      Se.Set Beam.customerNammaTags ((fmap (.getTagNameValueExpiry) <$>) customerNammaTags),

...
processWaypoints ::
  ...
  Maybe Meters ->
  m ()
processWaypoints ih@RideInterpolationHandler {..} driverId ending estDist estTollCharges estTollNames pickupDropOutsideThreshold rectifyDistantPointsFailureUsing isTollApplicable enableTollCrossedNotifications passedThroughDrop waypoints thresholdForSnapToRoadCall = do
  ...
  wrapDistanceCalculation driverId $ do
    ...
    if isJust rectifyDistantPointsFailureUsing
      then do
        ...
        when (length totalWaypoints > 1) do
          let threshold = fromMaybe 500 thresholdForSnapToRoadCall
          processWaypointsWithSnapToRoadCheck totalWaypoints threshold calculationFailed
  ...
  where
    processWaypointsWithSnapToRoadCheck points threshold calculationFailed = do
      case points of
        [] -> return ()
        [_] -> return ()
            ...
            foldM
              ( \(accDist, accMaxGap, prevPoint) currPoint -> do
                  let dist = highPrecMetersToMeters $ distanceBetweenInMeters prevPoint currPoint
                      ...
                      newAccDist = if newMaxGap <= threshold then accDist + dist else accDist

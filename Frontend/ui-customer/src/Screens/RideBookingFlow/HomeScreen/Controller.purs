...
eval (SearchLocationModelActionController (SearchLocationModelController.EditTextFocusChanged textType)) state = do
  ...
  if state.data.fareProductType == FPT.AMBULANCE && textType == "D" && state.props.firstTimeAmbulanceSearch then
   validateSearchInput state { props { isSource = if state.props.currentStage == ConfirmingLocation && state.props.isSource == Just true then Just true else Just false, searchLocationModelProps{crossBtnDestVisibility = (STR.length state.data.destination) > 2}}, data {source = if state.data.source == "" then state.data.searchLocationModelData.prevLocation else state.data.source, locationList = if state.props.isSource == Just false then state.data.locationList else state.data.destinationSuggestions } } "Hospital"
...
eval (LocationTagBarAC (LocationTagBarV2Controller.TagClicked tag)) state = do
  case tag of
    ...
    "AMBULANCE" ->
      let
        updatedState = state { data { fareProductType = FPT.AMBULANCE} , props {firstTimeAmbulanceSearch = true , types_ = Just "hospital"} }
      in
